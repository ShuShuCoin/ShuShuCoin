...
class StatusHandler(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  if self.path=='/status.json':body=json.dumps(data,indent=2).encode();ctype='application/json'
   ...
   body=("<html><head><meta charset=utf-8><meta http-equiv=refresh content=10><title>ShuShuCoin Pool</title><style>body{background:#101114;color:#f6e5ae;font:16px sans-serif;margin:0}main{max-width:1100px;margin:auto;padding:30px}.card{display:inline-block;background:#1b1c22;border:1px solid #6f5320;border-radius:12px;padding:16px;margin:6px;min-width:150px}b{font-size:24px;color:#ffd369}table{width:100%%;border-collapse:collapse;background:#191a20}td,th{padding:12px;border-bottom:1px solid #332d25;text-align:left}a{color:#ffd369}</style></head><body><main><h1>ShuShuCoin Pool</h1><p>Live mainnet mining status - refreshes every 10 seconds | <a href=/status.json>JSON API</a> | <a href=http://106.13.140.227:3335/>Block Explorer</a></p><div class=card>Height<br><b>%s</b></div><div class=card>Issued SHUSHU<br><b>%s</b></div><div class=card>P2P peers<br><b>%s</b></div><div class=card>Active miners<br><b>%s</b></div><div class=card>Reported H/s<br><b>%s</b></div><div class=card>Accepted blocks<br><b>%s</b></div><p>Difficulty: %s | Stale submits: %s | P2P peers are nodes; pool role is not announced by the protocol.</p><h2>Miner activity</h2><table><tr><th>Address</th><th>Status</th><th>H/s</th><th>Blocks</th><th>Stale</th><th>Last seen</th></tr>%s</table></main></body></html>" % (data["chain"]["height"],data["chain"]["issued_supply_shushu"],data["chain"]["p2p_peer_count"],data["active_miners"],data["reported_hashrate_hs"],data["pool"]["blocks_accepted"],data["chain"]["difficulty"],data["pool"]["stale_submissions"],rows)).encode();ctype="text/html; charset=utf-8"
